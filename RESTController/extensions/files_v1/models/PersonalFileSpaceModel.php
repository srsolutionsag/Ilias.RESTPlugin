...
class PersonalFileSpaceModel extends Libs\RESTModel
{
     ...
     */
    public function handleFileUploadIntoMyFileSpace($file_upload, $user_id, $owner_id = 13, $destinationFolderStr = "abc")
    {
    ...
    protected function putObjectInMyFileSpaceTree($a_obj, $user_id, $destinationFolderStr)
    {
        ...
        $ilLog = $container->logger()->root();
        $rbacreview = $container->rbac()->review();
        ...
        if ($destinationFolderStr != '') { // create folder if necessary
            ...
            foreach ($aNodeIdsFolders as $nodeId) {
                $ilLog->debug("Checking personal workspace folders. Found Node_ID " . $nodeId . "");
                ...
                if ($title == $destinationFolderStr) {
                    $ilLog->debug("Folder $destinationFolderStr found.");
            ...
            if ($destFolderExists == false) {
                ...
                $ilLog->debug("Folder $destinationFolderStr not found. Creating it ...");
